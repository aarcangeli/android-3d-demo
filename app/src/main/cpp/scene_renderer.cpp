...
void SceneRenderer::buildCylinderMesh() {
    ...
    for (int i = 0; i < segs; ++i)
        idx.insert(idx.end(), {topCenter,(uint16_t)(topRingStart+i+1),(uint16_t)(topRingStart+i+1)});
    ...
    for (int i = 0; i < segs; ++i)
        idx.insert(idx.end(), {botCenter,(uint16_t)(botRingStart+i),(uint16_t)(botRingStart+i)});
